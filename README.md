...
耗时下方还有一个同样默认折叠的「模型输出原文」面板，展开是模型返回的未改动原文。
...
「模型输出原文」面板取自 `src/fire_safety/model_output.py`。捕获点是 `qwen.analyze_image` 拿到
响应、尚未解析的那一刻，所以 JSON 解析失败或 Schema 校验失败时同样能看到原文——这类失败恰恰是
最需要看原文的场合。原文与面板之间只隔一层 HTML 转义，不做换行、缩进或字段的整理，读者可以直接
和上方结论对照，判断是模型答错还是回答忠实但被后续步骤丢弃。捕获同样走 `ContextVar`，没有开启
捕获时是空操作。

终端日志需要 INFO 级别，`app.py` 已显式配置（Gradio 与 uvicorn 都不配置 root logger，
缺省只输出 WARNING 以上）。
...
| F07 | 已完成 | 每次点击的耗时分段：结果区折叠面板（含模型输出原文）+ 终端 `[耗时]` 日志 |
