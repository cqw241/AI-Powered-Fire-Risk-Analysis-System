...
启动时扫描 `data/legal/risk_packs` 下所有启用规则包，并将每个包的以下文件加载到内存：
...
法规数据按领域放在 `data/legal/risk_packs/<pack-id>/` 中。每个规则包包含
`manifest.json`、`clauses.json`、`issue_codes.json` 和 `rule_bindings.json`；`data/legal`
顶层不保留旧版三文件。
